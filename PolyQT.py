...
from model_quantum import PolymerModel
...
class DownstreamRegression(nn.Module):
    def __init__(self, drop_rate=0.1):
        ...
        self.QuantumModel = deepcopy(QuantumModel)

        self.Regressor = nn.Sequential(
            nn.Dropout(drop_rate),
            nn.Linear(self.QuantumModel.embed_dim, self.QuantumModel.embed_dim),
            nn.ReLU(),
            nn.Linear(self.QuantumModel.embed_dim, 1)
        )

    def forward(self, input_ids, attention_mask):
        outputs = self.QuantumModel(x=input_ids, mask=attention_mask)
...
def main(finetune_config):
    ...
    optimizer = AdamW(
        [
            {"params": model.QuantumModel.parameters(), "lr": finetune_config['lr_rate'],
             "weight_decay": 0.0},
...
if __name__ == "__main__":
    finetune_config = yaml.load(open("config_PolyQT.yaml", "r"), Loader=yaml.FullLoader)
    ...
    QuantumModel = PolymerModel(embed_dim=args.embed_dim,
